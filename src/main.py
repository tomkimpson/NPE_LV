...
try:
    ...
    from .clinical_data import ClinicalStudy, validate_clinical_data_compatibility
    from .teirv_utils import create_teirv_prior, create_teirv_time_grid, teirv_parameter_summary, get_teirv_initial_conditions, apply_observation_model
    from .teirv_simulator import gillespie_teirv
except ImportError:
    # Fall back to direct imports (for direct execution)
    from teirv_data_generation import TEIRVDataGenerator
    from teirv_inference import TEIRVInference
    from clinical_data import ClinicalStudy, validate_clinical_data_compatibility
    from teirv_utils import create_teirv_prior, create_teirv_time_grid, teirv_parameter_summary, get_teirv_initial_conditions, apply_observation_model
...
class TEIRVWorkflow:
    ...
    def setup_workflow_directory(self, workflow_name: Optional[str] = None) -> Path:
        ...
        self.workflow_dir = Path(f"results/experiments/production_runs/{workflow_name}")
...
Examples:
  ...
  # Complete pipeline
  python src/main.py full --n_samples 20000
        """
    )
    ...
    gen_parser.add_argument('--output', type=str, default='data/processed/teirv_training_data.pkl', help='Output filepath')
    ...
    train_parser.add_argument('--output', type=str, default='models/production/teirv_npe_model.pkl', help='Output model path')
    ...
    inference_parser.add_argument('--output', type=str, default='results/predictions/inference_results', help='Output directory')
